...
category.get('/all/isActive/:activeness', function (req, res) {
  ...
  if (activeness !== null) {
    knex.select().from('Category').where('isActive', activeness)
      .then((data) => {
        res.status(200).send(data).end();
      })
      .catch((error) => {
...
category.get('/all/budgetLimit/:limit/:over', function (req, res) {
  if (isNaN(req.params.limit)) {
    ...
    knex.select().from('Category').where('budgetLimit', '>', req.params.limit)
      .then((data) => {
        res.status(200).send(data).end();
      })
      .catch((error) => {
...
category.get('/:id', function (req, res) {
  knex.select().from('Category').where('id', req.params.id)
    .then((data) => {
      if (data.length == 0) {
...
category.post('/', function (req, res) {
  if (!req.body.name) {
    res.status(400).send("Category name is missing!").end();
  } else {
    knex.insert(req.body).into('Category')
      .then((data) => {
        res.status(200);
        res.send(data);
      })
      .catch((error) => {
        if (error.errno == 1062) { // https://mariadb.com/kb/en/library/mariadb-error-codes/
          res.status(409).send("Category with that name already exists!").end();
        } else {
          res.status(500).send("Database error: " + error.errno).end();
        }
      });
...
category.put('/', function (req, res) {
  if (!req.body.id || !req.body.name) {
    res.status(400).send("Category id or name are missing!").end();
  } else {
    knex('Category').where('id', req.body.id).update(req.body)
      .then((data) => {
        if (data == 0) {
          res.status(404).send("Invalid row number: " + req.body.id).end();
        } else {
